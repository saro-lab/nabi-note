---
title: 基本的な使い方
description: npm で導入して nabi オブジェクトをひとつ立て、四つの入力と三つの出力でドキュメントをやり取りします。
---
...
何もないエディタは最初の行に案内文をうっすらと立てます。文字が一文字入った瞬間に消え、
消して再び空になれば再び立ちます。**何もしなくても現れます** — 言葉はコア辞書のもので、
その mount の言語に従います。場所は**文章の向き**が決めます(LTR なら左、RTL なら右) —
その行が中央・右揃えでも案内文はそれに従いません。

```ts
mountSurface({ nabi, registry, root: surface, placeholder: 'ここにメモを残してください' })
mountSurface({ nabi, registry, root: surface, placeholder: '一行目\n二行目' })   // 複数行
mountSurface({ nabi, registry, root: surface, placeholder: '' })   // 案内文なし
```

改行(`\n`)はそのまま行になります。ただし案内文は**フローの外**に立つので(キャレットを
押さないように)、編集領域が一行の高さだと複数行の案内文は下にあふれます — 複数行を
使うなら編集領域にその分の最小高さを与えてください。

言葉は編集領域ルートの `--nabi-placeholder` に入り、描くのはスタイルシートです。色や
質感を変えるにはこの規則を書き換えてください。
...
付けません。少しずれた値は拒否する代わりに **読みながら直します** — 空の表セル、行では
ない表の子、あふれた結合といったものがそれで、危険なアドレスが濾し取られるのも同じ
手順です。拒否はまったく読めない形のためのものです。そして値を読む途中で例外が起きても
エディタは止まりません — 拒否(`false`)に変わり、`console.error` で何が拒否されたのかを
知らせます。

| 答えが `false` になる場所 | |
|---|---|
| `setJson` | ナビツリーの形式でない(空の値は除く — 下記) |
| `setHtml` | `parseHtml` アダプタを差していない(下記)、または編集がロックされている(空の値は除く) |
| `applyCommand` | そのコマンドがない、または **何も変わらない** |

**空のドキュメントの姿はひとつです — `[{"w":"p","ch":[]}]`。** 全選択して削除するように
文章をまるごと消した場所には、最初のブロックの見出し・揃えが残りません。複数行のうち
一行だけを空にしたのは違います — その行を書き直すつもりなので、その段落の属性はそのまま
残ります。

**空の値は形式エラーではなく空のドキュメントです。** `null`・`undefined`・空の文字列
(空白だけのものも)・空の配列を渡すと、拒否せず **空の画面に落ち着いて `true`** を答えます
— `setJson`・`setHtml` どちらもそうで、だから「空にする」はいつも成功します。空の値は
読むものがないので、`setHtml` はそのときアダプタ(下記)も要りません。形の違う値はそのまま
拒否です — 空のものと違う形のものは別です。
