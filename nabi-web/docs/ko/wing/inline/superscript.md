...
`superscriptWing`은 `<sup>`의 소유자(claim)입니다. 단위의 제곱이나 각주 번호에
씁니다.

- 인정하는 태그는 `<sup>` 하나입니다. 속성은 살리지 않습니다.
- 힌트 모드 단축키도 가속키도 없습니다(업로드처럼 배지가 안 뜨는 wing 중 하나).
  툴바 무리는 `script` 로, 아랫첨자와 나란히 서되 등록 순서대로 이쪽이 먼저입니다.
- 글자를 고른 채 누르면 토글입니다.
...
**이 시트는 아랫첨자와 나눠 쓰는 한 벌입니다.** 두 wing 이 같은 글자를 들고 있어, 둘 다
등록해도 문서에는 **한 번만** 실립니다(`collectSheets` 가 같은 글의 시트를 걷어 냅니다).
저장값(HTML)에는 `<sup>` 태그만 남고 스타일 자체는 안 실립니다.
